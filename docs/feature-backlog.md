...
## Summary Table
| Feature | Priority | Status | Impact Snapshot |
| --- | --- | --- | --- |
| Realm Gates campaign flow | High | Pending | Adds structured progression with handcrafted encounter mixes |
| Elemental power-ups & resource loop | High | Implemented | Introduces moment-to-moment decision making and mid-run variety |
| Relic crafting & loadouts | High | Pending | Unlocks pre-run strategy with customizable relic combinations |
| Elite hazards & boss encounters | Medium | Backlog | Creates new spike moments once baseline mastery is reached |
| Daily trials & rotating mutators | Medium | Backlog | Encourages daily engagement and leaderboard churn |
| Achievements & cosmetic skins | Medium | Backlog | Rewards mastery, supports long-term retention |
| Accessibility assists & comfort options | Medium | Backlog | Broadens audience and improves approachability |
| Dynamic weather & biomes | Medium | Implemented | Adds visual variety and momentary rule twists mid-run |
| Async leaderboards & share codes | Low | Backlog | Enables social proof without real-time backend |
| Ghost replays & training mode | Low | Backlog | Gives players tools to self-improve and share runs |
| Seasonal festival events | Low | Backlog | Provides limited-time goals and cosmetics tied to calendar beats |
...
- **Follow-ups:** Tune shard thresholds, buff durations, and HUD messaging after playtests; explore additional buff variants for the Phase 6 roadmap.
...
- **Status:** Pending (ticket drafted in `docs/tickets/phase6-high-priority.md`).
...
- **Status:** Implemented on main (weather cycles active in-play & menu).
